...
static program_info programs[] = {
    ...
    {.name = "test-mm", .ptr = (uint64_t) &test_mm, .min_args = 0, .max_args = 0},
};
...
char ** make_params(char ** words, unsigned int len){
    void * coso = sys_alloc((2 + len) * sizeof(char *)); // + 1 for name, + 1 por null termination
